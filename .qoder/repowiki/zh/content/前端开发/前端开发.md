...
- [packages/web/src/pages/BusinessArchitecturePage.tsx](file://packages/web/src/pages/BusinessArchitecturePage.tsx)
...
- [packages/web/src/hooks/useBusinessArchitecture.ts](file://packages/web/src/hooks/useBusinessArchitecture.ts)
...
- [packages/web/src/contexts/BusinessArchitectureContext.tsx](file://packages/web/src/contexts/BusinessArchitectureContext.tsx)
...
- [packages/web/src/components/business-architecture/ArchitectureTree.tsx](file://packages/web/src/components/business-architecture/ArchitectureTree.tsx)
- [packages/web/src/components/business-architecture/ProcessMapping.tsx](file://packages/web/src/components/business-architecture/ProcessMapping.tsx)
- [packages/web/src/components/business-architecture/PropertyPanel.tsx](file://packages/web/src/components/business-architecture/PropertyPanel.tsx)
...
BA["BusinessArchitecturePage.tsx"]
end
subgraph "组件层"
subgraph "布局"
...
subgraph "业务架构"
BA_TREE["ArchitectureTree.tsx"]
BA_PROCESS["ProcessMapping.tsx"]
BA_PANEL["PropertyPanel.tsx"]
end
subgraph "项目"
...
CTX_BA["BusinessArchitectureContext.tsx"]
...
HK_BA["useBusinessArchitecture.ts"]
...
- 页面组件：围绕业务域划分，如项目列表、项目详情、仪表盘、领域模型、组织、角色、外部实体、应用、菜单管理以及新增的业务架构页面。
- 布局组件：页头导航与侧边栏，统一导航与面包屑。
- 通用组件：分页、数据表格容器、空状态、状态徽章等复用组件。
- 领域模型组件：编辑器、画布、工具箱与各类对话框。
- 业务架构组件：交互式树形可视化、拖拽流程映射、属性面板等。
- 项目组件：表格、卡片、归档确认等。
- UI 基础组件：基于 shadcn/ui 的按钮、对话框、表格、输入、选择、侧边栏、徽章、卡片、下拉菜单、警告对话框、复选框、标签、骨架屏、通知、开关、标签页、文本域、提示等。
- 上下文与钩子：项目上下文、领域模型上下文、业务架构上下文；项目列表、项目详情、应用、组织、领域模型、业务架构、角色行为、外部实体行为、主题切换等自定义 Hook。
...
### 状态管理与组件通信
- 上下文：ProjectContext、DomainModelContext 与 BusinessArchitectureContext 提供跨层级共享的状态与动作，避免深层传递。
- 自定义 Hook：useProjectList、useProjectDetail、useApplications、useOrganization、useDomainModel、useBusinessArchitecture、useRoleBehavior、useExternalEntityBehavior、useThemeToggle 将数据获取与状态逻辑抽象为可复用单元，便于测试与维护。
...
### 业务架构组件详解
**新增** 业务架构模块提供了完整的交互式业务建模能力，包括树形结构可视化、拖拽流程映射和属性编辑面板。

#### 交互式树形可视化
ArchitectureTree 组件实现了业务架构的树形展示，支持节点展开/折叠、搜索过滤、右键菜单操作等功能。每个节点都携带完整的业务元数据，支持点击选中和高亮显示。

#### 拖拽流程映射
ProcessMapping 组件提供了可视化的业务流程绘制能力，支持从 ArchitectureTree 中拖拽节点到画布区域，自动建立节点间的关联关系。内置多种连接线类型和布局算法，支持流程图的重排和导出。

#### 属性面板
PropertyPanel 组件提供选中节点的详细属性编辑界面，支持字段增删改查、数据类型配置、验证规则设置等操作。与主画布保持实时同步，修改立即反映在可视化视图中。

```mermaid
graph TB
subgraph "业务架构页面"
BA_PAGE["BusinessArchitecturePage.tsx"]
end
subgraph "核心组件"
TREE["ArchitectureTree.tsx"]
PROCESS["ProcessMapping.tsx"]
PROPERTY["PropertyPanel.tsx"]
end
subgraph "状态管理"
BA_CTX["BusinessArchitectureContext.tsx"]
BA_HOOK["useBusinessArchitecture.ts"]
end
subgraph "数据交互"
API["API客户端"]
BACKEND["后端服务"]
...
### 领域模型画布增强
**更新** 领域模型画布交互得到了显著增强，包括改进的属性面板和更流畅的用户体验。

#### 增强的画布交互
ERCanvas 组件现在支持更丰富的交互操作，包括多选节点、批量操作、智能对齐和网格吸附功能。画布缩放和平移更加流畅，支持键盘快捷键操作。

#### 改进的属性面板
属性面板现在支持动态表单生成，根据选中实体的字段类型自动渲染相应的编辑控件。支持字段验证、默认值设置、关联关系配置等高级功能。
...
PAGES --> BUSINESS_ARCH["components/business-architecture/*"]
...
- 业务架构问题：检查 BusinessArchitectureContext 状态是否正确初始化；验证拖拽事件处理逻辑。
...
## 结论
本指南从技术栈、项目结构、组件体系、状态管理、路由导航、UI 规范、API 集成、性能优化、测试与部署等方面，为 AI 原型管理系统前端提供了系统化的开发指引。新增的业务架构模块和增强的领域模型画布功能进一步提升了系统的可视化建模能力。建议团队在日常开发中遵循本文规范，持续完善测试与监控体系，确保前端质量与交付效率。
