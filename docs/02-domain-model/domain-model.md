...
│                                //   "association"    — 普通关联：A 与 B 之间存在持久的对称结构关联（双向，如订单↔用户）
│                                //   "dependency"     — 依赖：A 临时使用 B，关系短暂，无持久引用（单向 A→B）
│                                //   "aggregation"    — 聚合：A 包含 B（整体-部分），但 B 可独立存在（单向）
│                                //   "composition"    — 组合：A 包含 B，B 随 A 生命周期结束（强拥有，单向）
│                                //   "generalization" — 泛化：A 是 B 的子类型（is-a），source=子类，target=父类（单向）
...
| kind | 语义 | 方向性 | 基数 | 生命周期绑定 | 从属关系 | 典型场景 |
|------|------|:-----:|:----:|------------|---------|---------|
| `association` | A 与 B 对称关联 | **双向** | 可配置 | 无 | 无 | 订单↔用户、商品↔分类 |
| `dependency` | A 临时使用 B | 单向 A→B | 可配置 | 无 | 无 | 服务A调用服务B |
| `aggregation` | A 包含 B（弱） | 单向 整体→部分 | 可配置 | 弱 | 有（整体-部分） | 部门→员工 |
| `composition` | A 包含 B（强） | 单向 整体→部分 | 可配置 | 强（B 随 A 消亡） | 有（整体-部分） | 订单→订单明细 |
| `generalization` | A 是 B 的子类型 | 单向 子类→父类 | 固定 1:1 | 无 | 无 | 企业客户→客户、左通道站→换电站 |

> **方向性与唯一性约束**（v1.6）：
> - `association` 双向：Canvas 渲染为无箭头实线；唯一性按无序对 `{A,B}+kind` 判定，即 `(A,B,association)` 与 `(B,A,association)` 视为同一条关系
> - 其余四类单向：Canvas 渲染为带特殊末端标记（箭头/菱形/三角）；唯一性按有序三元组 `(A,B,kind)` 判定
...
| 变更项 | 内容 | 影响 |
|--------|------|------|
| **association 方向性调整** | §5.2 RelationDef.kind `association` 由单向（A 持久引用 B）调整为双向对称（A 与 B 相互可导航的持久结构关联） | Canvas 渲染为无箭头实线；与 UML 标准 association 对齐 |
| **关系方向性列** | §5.2 语义对比表新增"方向性"列，明确 5 种关系的方向语义 | 便于后端唯一性校验与前端渲染分支判断 |
| **唯一性约束分档化** | `association` 采用无序对 `{A,B}+kind` 唯一；其余四类保持有序三元组 `(A,B,kind)` 唯一 | 转发至 PRD §4.3.3 B-M2-08 详细规则 |

> **触发原因**：PM 确认 association 本质上描述两个实体之间的双向导航关系（用户能看到自己的订单，订单也能回溯用户），单向箭头会造成语义偏差与设计师心智模型不一致。同时引入 P2 多关系约束（一对实体 + kind 唯一），避免同类型关系重复建立。
