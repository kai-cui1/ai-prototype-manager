...
| **M3** 业务流程 | 流程建模（节点池/边/决策/子流程） | 22 | ✅ | ✅ | ⏳ | ✅ | ✅ | ✅ | ✅ | S6 后端+前端完成；S7 API 测试 69 TC 全绿 + Code Review 通过 (2026-07-15)，S5 测试设计补录 m3-api-test-design.md |
| **M4** 业务架构 | 不限层数架构树 + 架构→流程映射 | 11 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ⏳ | S2~S5 全部完成 (2026-06-12)；54 TC 用例，AC/规则/异常 100% 覆盖 |
| **M6** 团队/用户/权限 | 多团队协同 + JWT/PAT 双重认证 + 三层角色 | 28 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | S6 API + Web 前端完成；S7 API 测试 78 TC 全绿（含 F-M6-04 CLI 3 TC + F-M6-Hardening）+ 浏览器实测 7 页面通过 (2026-07-15) |
| **菜单** 导航菜单 | Sidebar 菜单 CRUD（轻量配置） | ~6 | ✅ | ✅ | ⏳ | ✅ | ✅ | ⏳ | ⏳ | 可并入 M1 |

---

## §3.1 M6 团队/用户/权限 — 功能进展总览

> **API 端点组**：`auth.ts`（6）+ `users` (admin 内)（3）+ `teams.ts`（10）+ `shares.ts`（5）+ `tokens.ts`（3）+ `admin.ts`（6）= **约 28 个**

### 进度主表（功能 × SDLC 阶段）

| 功能编号 | 名称 | P | S0 | S1 | S2 | S3 | S4 | S5 | S6 | S7 | 备注 |
|----------|------|:-:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|------|
| **F-M6-01** | 用户登录（JWT + 5次锁定） | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | 包含在 f-m6-01-auth（20 TC） |
| **F-M6-02** | 用户登出 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-03** | 修改密码（PAT 联动 revoke） | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-04** | 重置密码(CLI) | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-04-reset-password（3 TC），CLI: `reset-admin-password` |
| **F-M6-05** | 首登强制改密 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-06** | 创建用户（SuperAdmin） | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-06-user（9 TC） |
| **F-M6-07** | 禁用/启用用户 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-08** | 创建团队 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-08-team（20 TC） |
| **F-M6-09** | 管理团队信息 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-10** | 解散团队 | P2 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-11** | 邀请成员 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-12** | 移除成员 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-13** | 变更成员角色 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-14** | 退出团队 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-15** | 共享项目（团队/用户） | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-15-share（10 TC） |
| **F-M6-16** | 撤销共享 | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-17** | 变更共享角色 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-18** | 创建 PAT（10个上限） | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-18-token（8 TC） |
| **F-M6-19** | 撤销 PAT | P0 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-20** | PAT 列表 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
| **F-M6-21** | 角色-权限矩阵配置 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | f-m6-21-admin（8 TC） |
| **F-M6-22** | 审计日志查询 | P1 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | |
...
| 维度 | 总数 | S5完成 | S6完成 | S7完成 | ⏳ | S5完成率 | S6完成率 | S7完成率 |
|------|:----:|:------:|:------:|:------:|:--:|:--------:|:--------:|:--------:|
| 功能点 | 22 | 22 | 21 | 21 | 1 | **100%** | **95%** | **95%** |

### M6 核心技术决策

1. **双轨认证**：JWT（15min）+ PAT（sha256 哈希、前缀 `apm_pat_`、不可逆）
2. **三层角色体系**：PlatformRole × TeamRole × ProjectRole
3. **权限判定算法**：SuperAdmin 直通 → 平台级 → 团队级 → 项目级（归属团队/共享给用户/共享给团队）
4. **默认拒绝硬化**（F-M6-Hardening，2026-07-15 完成）：过渡期放行已移除 —— 无 token → 401；`config.requires === undefined` → 403 兜底；M1~M4 实体路由通过 `applyDefaultEntityPermissions`（onRoute 钩子）批量声明 GET→`entity.read` / 写→`entity.write` + `:projectId` 作用域；项目路由显式声明 PROJECT_READ/WRITE/DELETE；个人项目 `createdBy` 创建者直通；resourceScope 支持异步查库解析（公司单资源路由）；MCP 通过 `APM_API_TOKEN` 附加 PAT 认证头；全量测试 627/627 全绿 + Code Review 通过
5. **安全固化**（Code Review 修复）：refreshToken 不可作 accessToken、权限更新事务保护、公开路由显式标记 `config.public`

### M6 前端浏览器实测（2026-07-15）

以 SuperAdmin (`admin@apm.local`) 登录后逐页验证 UI 渲染、Console 报错、5xx 请求：
...
| 3 | 我的团队（列表） | `/teams` | ✅ |
| 4 | 团队详情 | `/teams/:teamId` | ✅ |
| 5 | Access Token (PAT) | `/settings/tokens` | ✅ |
| 6 | 用户管理（SuperAdmin） | `/admin/users` | ✅ |
| 7 | 角色权限配置（SuperAdmin） | `/admin/permissions` | ✅ |

所有页面 UI 布局正常、无 Console 报错、无 5xx 网络请求。
...
| 类别 | 文件数 | 用例数 | 通过率 |
|------|:-----:|:-----:|:-----:|
| API 测试 | 40 | 630 | 100%（F-M1: 355 + M2: 55 + M3: 69 + M4 + M6: 78 + OpenAPI: 7 + 其他，2026-07-15 全量全绿） |
