...
> **适用范围**：Step 2（产品 PRD 设计）的产出物格式规范
> **文件存放位置**：见 CLAUDE.md「项目目录规范」中 `03-prd-ux/` 的子目录说明
...
### 4.4 UI 文案与原型一致性 **（必须遵守）**

> **新增规则（v1.1）**：源于 M1 模块 CreateProjectDialog 实现与原型偏差的教训。

**问题背景**：PRD §4.X.2 页面设计元素表中定义的用户可见文案（label/placeholder/button）可能与 HTML 高保真原型中的实际文案不一致。开发者以 PRD 为参考源时会导致实现与原型视觉偏差。

**规则**：

- [ ] **§4.X.2 元素表中的用户可见文案，必须标注「见原型」或明确写出原型中的实际文案**
  - 正确写法：`name 输入框 | Input | ✅ | 项目名称（见原型 L443），placeholder: 如：ecommerce-admin`
  - 错误写法：`name 输入框 | Input | ✅ | 编程标识符, placeholder: 如 my-project`（仅写了 PRD 术语，未对齐原型）
- [ ] **如果 PRD 和原型对同一元素的文案不同，在 PRD 元素表的「说明」列注明差异和裁决**
  - 格式：`| 说明 | 原型用"项目名称"，本文用"编程标识符"（技术术语），实现时以原型为准 |`
- [ ] **§6.3 UI/UX 验收中的文案类验收项，预期值取自原型 HTML 而非 PRD**

**裁决原则速查**：

| 文案类型 | 以谁为准 | 理由 |
|---------|---------|------|
| 标题 / label / placeholder / button 文字 | **原型** | 用户直接看到的是原型定义的视觉 |
| hint / 提示文字 | **原型** | 属于用户引导文案 |
| 错误提示 / 校验消息 | **PRD** | 技术性提示，面向开发者调试 |
| 内部字段名 / 数据格式 | **PRD** | 数据模型定义 |
