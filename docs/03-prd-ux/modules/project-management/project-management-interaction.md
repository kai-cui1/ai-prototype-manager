...
| 模块概要区域 | 概要卡片网格 | `Card` | 每张卡片：模块图标 + 统计数；**整张卡片可点击**，跳转对应管理模块 |
| 概要卡片 | 应用管理卡片 | `Card` | 统计：总应用数 + 各类型分布标签（仅展示 > 0 的类型，最多 4 个）；点击跳转 `/p/:id/applications` |
| 概要卡片 | 领域模型卡片 | `Card` | 统计：N 个实体；Phase 1 点击显示 Toast "领域模型功能正在开发中" |
| 概要卡片 | 业务流程卡片 | `Card` | 统计：N 个流程；Phase 1 点击显示 Toast "业务流程功能正在开发中" |
| 概要卡片 | 公司/组织卡片 | `Card` | 统计：N 家公司；点击跳转 `/p/:id/organization` |
| 概要卡片 | 部门卡片 | `Card` | 统计：N 个部门；点击跳转 `/p/:id/organization` |
| 概要卡片 | 角色卡片 | `Card` | 统计：N 个角色；点击跳转 `/p/:id/roles` |
| 概要卡片 | 外部实体卡片 | `Card` | 统计：N 个；点击跳转 `/p/:id/external-entities` |

### 4.3 概要卡片点击交互（F-M1-10）

所有模块概要卡片均为**可点击卡片**，hover 时显示阴影提升（`hover:shadow-md`），cursor 切换为 pointer，点击后进入对应管理模块。

| 卡片 | 点击目标路由 | Phase 1 处理 |
|------|------------|-------------|
| 应用管理 | `/p/:projectId/applications` | 已实现，正常跳转 |
| 公司/组织 | `/p/:projectId/organization` | 已实现，正常跳转 |
| 部门 | `/p/:projectId/organization` | 已实现，正常跳转 |
| 角色 | `/p/:projectId/roles` | 已实现，正常跳转 |
| 外部实体 | `/p/:projectId/external-entities` | 已实现，正常跳转 |
| 领域模型 | `/p/:projectId/domain-model` | 已实现，正常跳转 |
| 业务流程 | `/p/:projectId/business-processes` | Phase 1 占位：点击弹出 Toast "业务流程功能正在开发中" |

**卡片可点击视觉规范：**
- `cursor-pointer`
- `transition-shadow hover:shadow-md`
- hover 时卡片阴影提升，增强可点击感知
...
| 查看模式 | 所有表单控件只读（disabled），隐藏「添加参数」「删除」按钮，Footer 仅保留「关闭」按钮；Dialog 标题显示「查看 Action」 |

### 11.5 Decision Dialog（新建 / 编辑 / 查看）
...
| 查看模式 | 所有表单控件只读（disabled），隐藏「添加参数」「删除」「添加分支」按钮，隐藏分支删除按钮；Footer 仅保留「关闭」按钮；Dialog 标题显示「查看 Decision」 |
