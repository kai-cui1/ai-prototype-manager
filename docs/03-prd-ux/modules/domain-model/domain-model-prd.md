...
- **领域（DomainBoundary）** — 业务子域的分组容器（如 "订单域"、"用户域"），实体可归属到某个领域
...
| F-M2-06 | 领域管理 | P0 | 领域边界的创建、编辑、删除；实体归属/脱离领域（Canvas 拖入/拖出） | §4.6 |
...
> **优先级**：P0
> **前置功能**：F-M2-01（实体管理）
> **交互设计**：→ `domain-model-interaction.md#§3.8`

#### 4.6.1 涉及的领域模型

| 实体 | 用途 | 关键字段 | 引用 |
|------|------|---------|------|
| domain_boundaries | 领域边界主表 | id, project_id, name, description, config | `phase1-database-schema.md#表X`（新增） |
| domain_entities | 实体归属领域 | id, domain_id | `phase1-database-schema.md#表2`（新增 domain_id 列） |

**实体关系图**：

```mermaid
erDiagram
    Project ||--o{ DomainBoundary : "has"
    DomainBoundary ||--o{ Entity : "contains (0:N)"
    Entity }o--o| DomainBoundary : "belongs to (optional)"
```
...
| 步骤 | 业务动作 | 输入 | 输出 | 前置条件 | 异常处理 |
|------|---------|------|------|---------|---------|
| 1 | 创建领域 | project_id, name, description? | 新领域对象（含默认 canvas_position） | 项目存在 | 名称重复 → 409 |
| 2 | 编辑领域 | domain_id, name?, description?, canvasPosition? | 更新后的领域 | 领域存在 | 领域不存在 → 404 |
| 3 | 删除领域 | domain_id | 删除成功确认；所有归属实体的 domainId 置 null | 领域存在 | 领域不存在 → 404；需确认对话框 |
| 4 | 实体归属领域 | entity_id, domain_id | 更新后的实体（domainId 设为目标领域 ID） | 实体和领域存在且属于同一项目；实体当前无领域或属另一领域 | 实体已有领域 → 允许覆盖；跨项目 → 400 |
| 5 | 实体脱离领域 | entity_id, domain_id=null | 更新后的实体（domainId 置 null） | 实体存在且当前属某领域 | 实体不属于任何领域 → 无操作 |
| 6 | 查询项目内领域列表 | project_id | 领域列表 | 项目存在 | — |

**快捷操作**：

| 操作 | 触发方式 | 行为 | 确认机制 |
|------|---------|------|:--------:|
| Canvas 拖入实体到领域框 | ER 图中拖拽实体节点到领域框范围内松手 | 调用 API 更新 entity.domainId = domain.id | 无需确认（松手即归属） |
| Canvas 拖出实体脱离领域 | ER 图中拖拽实体节点离开领域框范围松手 | 调用 API 更新 entity.domainId = null | 无需确认 |
| Canvas 拖动领域框 | ER 图中拖拽领域框 | 领域框移动 + 内部实体同步偏移（保持相对位置） | 无需确认 |
...
| 字段 | 规则 | 错误提示 |
|------|------|---------|
| name | 非空，项目内唯一，1-128 字符 | "领域名称不能为空" / "该领域名称已被使用" |
| description | 可选，0-512 字符 | — |
| canvasPosition | 必须包含 x, y, width, height，均为数字 | "画布位置数据格式无效" |

**业务约束**：

| # | 规则 | 说明 |
|---|------|------|
| B-M2-F06-01 | 实体最多归属一个领域 | domainId 为可空 FK，不为 null 时唯一指向一个 DomainBoundary |
| B-M2-F06-02 | 允许空领域存在 | 领域可以不包含任何实体 |
| B-M2-F06-03 | 删除领域时实体 domainId 置 null（SET NULL） | 实体不随领域删除，仅解除归属关系 |
| B-M2-F06-04 | Canvas 上领域框不允许重叠 | 两个领域框的矩形区域不能相交（创建/移动/调整大小时校验） |
| B-M2-F06-05 | 拖入领域框松手即归属 | 实体中心点在领域框矩形内时建立归属，无需额外确认 |
| B-M2-F06-06 | 拖动领域框时内部实体跟随移动 | 保持实体与领域框的相对位置不变，松手后批量更新实体 canvas_position |
| B-M2-F06-07 | 领域框尺寸可手动调整 | 用户可拖拽领域框边角/边缘调整宽高 |

**异常场景**：

| 场景 | 触发条件 | 系统行为 |
|------|---------|---------|
| 领域名称重复 | 同一项目内已有同名领域 | 返回 409，提示名称已被使用 |
| 领域不存在 | domain_id 无效或已删除 | 返回 404 |
| 跨项目操作 | 实体或领域属于其他项目 | 返回 400 |
| 领域框重叠 | 移动/调整大小时与其他领域框相交 | 返回 409，提示领域框不可重叠；恢复原位置/尺寸 |
...
| 字段 | 类型 | 必填 | 默认值 | 校验规则 | 说明 |
|------|------|:----:|:------:|---------|------|
| name | string | ✅ | — | 非空，项目内唯一，1-128 字符 | 领域名称 |
| description | string | — | null | 0-512 字符 | 领域描述 |
...
|------|------|:----:|:------:|---------|------|
| name | string | — | — | 非空，项目内唯一，1-128 字符 | 领域名称 |
| description | string | — | — | 0-512 字符 | 领域描述 |
| canvasPosition | object | — | — | { x: number, y: number, width: number, height: number } | Canvas 位置与尺寸 |
...
| name | string | DB | 领域名称 |
| description | string | DB | 描述 |
| config | json | DB | 含 canvas_position（x, y, width, height）|
| entity_count | number | 关联查询 | 归属此领域的实体数量 |
| created_at | datetime | DB | 创建时间 |
| updated_at | datetime | DB | 更新时间 |
...
```typescript
interface ERGraphData {
  entities: ERNode[];       // 实体节点列表
  relations: EREdge[];      // 关系边列表
  domains: ERDomain[];      // 领域框列表（v1.2 新增）
}
...
ERNode 需扩展 `domainId` 字段：

```typescript
interface ERNode {
  // ... 现有字段不变
  domainId?: string;        // 归属的领域 ID（v1.2 新增；可空）
}
```

#### 4.6.5 AI 编码提示

- **[domain_boundaries 表新建]**：需新增迁移脚本，创建 `domain_boundaries` 表并添加 `domain_entities.domain_id` 可空 FK 列（ON DELETE SET NULL）
- **[Canvas 领域框重叠检测]**：移动/调整领域框大小时，需在前端做矩形相交检测。检测时机：拖拽过程中实时预览（视觉反馈），松手时校验（提交 API 前校验或后端校验）
- **[领域框拖拽联动]**：拖动领域框时需获取所有 `domainId === 该领域.id` 的实体节点，同步偏移 dx/dy。松手后批量调用实体位置更新 API
- **[ER Graph API 扩展]**：`getFullERGraph` 和 `getEntityERGraph` 返回数据需新增 `domains` 数组和 ERNode.domainId 字段
- **[domain_id FK 性能]**：`domain_entities.domain_id` 需添加索引（`idx_domain_entities_domain`），因为 Canvas 渲染和领域框拖拽联动需频繁查询某领域下的所有实体
...
| # | 规则 | 影响范围 | 说明 |
|---|------|---------|------|
| G-M2-01 | name 字段在各自范围内唯一 | F-M2-01, F-M2-02, F-M2-06 | 实体在项目内唯一，字段在实体内唯一，领域在项目内唯一 |
| G-M2-02 | name 必须符合编程标识符格式 | F-M2-01, F-M2-02 | 字母/数字/下划线，不能以数字开头 |
| G-M2-03 | 所有操作必须校验 project_id/entity_id/domain_id 的存在性和归属权 | 全部功能点 | 跨项目访问返回 404（不暴露存在性）|
| G-M2-04 | 字段类型必须是 Phase 1 支持列表中的值 | F-M2-02 | 见 §4.2.3 类型列表 |
| G-M2-05 | 关系三元组 (source, target, kind) 在项目内唯一 | F-M2-03 | 避免重复关系定义 |
| G-M2-06 | Canvas 上领域框不允许重叠 | F-M2-06 | 两个领域框的矩形区域不能相交 |
...
| AC-M2-29 | 创建领域成功 | F-M2-06 | 自动化 | 传入合法 name 返回 201 和新领域；数据库存在记录 |
| AC-M2-30 | 创建领域名称重复返回 409 | F-M2-06 | 自动化 | 同一项目内重复名称返回 409 |
| AC-M2-31 | 编辑领域更新指定字段 | F-M2-06 | 自动化 | PUT 后数据库字段更新，其他字段不变 |
| AC-M2-32 | 删除领域后实体 domainId 置 null | F-M2-06 | 自动化 | 删除领域后，归属该领域的所有实体 domainId 变为 null，实体不删除 |
| AC-M2-33 | 实体归属领域成功 | F-M2-06 | 自动化 | 更新 entity.domainId 后，实体详情中 domainId 为目标领域 ID |
| AC-M2-34 | 实体脱离领域成功 | F-M2-06 | 自动化 | 将 entity.domainId 置 null 后，实体不再归属任何领域 |
| AC-M2-35 | 领域框重叠返回 409 | F-M2-06 | 自动化 | 移动/调整领域框至与其他领域框重叠时返回 409 |
| AC-M2-36 | ER 图数据包含领域信息 | F-M2-04, F-M2-06 | 自动化 | 项目级 ER 图返回 domains 数组，实体节点包含 domainId 字段 |
